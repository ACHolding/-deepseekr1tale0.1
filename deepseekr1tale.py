...
class DeepSeekTale:
    ...
    def _make_beep(self, freq, duration):
        ...
        try:
            import numpy as np
            arr = (127 * np.sin(2 * np.pi * freq * np.arange(n_samples) / sample_rate)).astype(np.int8)
    ...
    def _update_overworld(self):
        ...
        px_tile = new_x // TILE_SIZE
        py_tile = new_y // TILE_SIZE
        px_tile2 = (new_x + TILE_SIZE - 1) // TILE_SIZE
        py_tile2 = (new_y + TILE_SIZE - 1) // TILE_SIZE

        can_move = True
        for tx in range(px_tile, px_tile2 + 1):
            for ty in range(py_tile, py_tile2 + 1):
                if (tx, ty) in room["walls"]:
                    can_move = False
        ...
        # Boundaries
        self.px = max(0, min(self.px, (room["w"] - 1) * TILE_SIZE))
        self.py = max(0, min(self.py, (room["h"] - 1) * TILE_SIZE))
    ...
    def _interact_npc(self, npc):
        ...
        if npc_type == "flowey" and not self.player.flowey_met:
            ...
            self._start_battle("toriel_boss")
            ...
            self._start_battle("mettaton_ex")
    ...
    def _draw_overworld(self):
        ...
        room = self._get_room()
        cam_x = self.px - SCREEN_W // 2
        cam_y = self.py - SCREEN_H // 2
        cam_x = max(0, min(cam_x, room["w"] * TILE_SIZE - SCREEN_W))
        cam_y = max(0, min(cam_y, room["h"] * TILE_SIZE - SCREEN_H))
            ...
            for y in range(room["h"]):
                ...
                if -TILE_SIZE < rx < SCREEN_W + TILE_SIZE and -TILE_SIZE < ry < SCREEN_H + TILE_SIZE:
                    if (x, y) in room["walls"]:
                        ...
                        floor_colors = [(5, 5, 25), (8, 8, 30), (6, 6, 28)]
                        pygame.draw.rect(self.screen, random.choice(floor_colors) if False else (8, 8, 28),
    ...
    def _start_dialogue(self, queue=None):
        ...
        self.dialogue_finished = False
        self.dialogue_show_arrow = False
        self.dialogue_is_battle = (self.state in (GameState.BATTLE, GameState.BATTLE_DIALOGUE, GameState.BATTLE_MENU))
    ...
    def _start_battle(self, enemy_id):
        ...
        self.soul_x = SCREEN_W // 2
        self.soul_y = SCREEN_H // 2

        # Battle box (where the soul moves)
    ...
    def _update_battle(self):
        ...
        # If player's action caused text, show message before enemy turn
        if self.battle_text_timer > 0 and not self.battle_player_turn and not self.battle_attack_phase:
        ...
        # Soul movement during attack
        if self.battle_attack_phase:
            self._update_soul()

        # Check for menu input
        if self.battle_player_turn and self.battle_phase not in ("intro", "attack"):
    ...
    def _update_soul(self):
        ...
        if len(self.bullets) == 0 and self.bullet_spawn_timer > 120:
            self.battle_attack_phase = False
            self.battle_player_turn = True
            self.turn_count += 1
    ...
    def _update_attack_phase(self):
        # Continuous bullet spawning
        if len(self.bullets) < 30 and self.bullet_spawn_timer % 10 == 0 and self.bullet_spawn_timer < 120:
            patterns = self.enemy_data.get("bullets", ["simple"]) if self.battle_enemy else ["simple"]
        ...
        # Check hits again for newly spawned bullets
        for b in self.bullets[:]:
            dist = math.hypot(self.soul_x - b.x, self.soul_y - b.y)
            if dist < self.soul_size + b.size:
                self._player_hit()
    ...
    def _player_hit(self):
        if not self.battle_attack_phase:
            return
        damage = max(1, self.enemy_data.get("atk", 5) - self.player.def_ // 2)
            ...
            if e.type == pygame.QUIT:
                ...
                if self.battle_phase == "menu":
                    if e.key in (pygame.K_LEFT, pygame.K_a):
                        self.battle_menu_selection = max(0, self.battle_menu_selection - 1)
                        self._play_sound("cursor")
                    elif e.key in (pygame.K_RIGHT, pygame.K_d):
                        self.battle_menu_selection = min(3, self.battle_menu_selection + 1)
                        self._play_sound("cursor")
                    elif e.key in CONFIRM_KEYS:
                        self._play_sound("confirm")
        ...
        for e in pygame.event.get():
            if e.type == pygame.QUIT:
                ...
                if e.key in CONFIRM_KEYS:
                    ...
                    bar_x = zone_center - 100 + self.fight_power * 2
                    ...
                    if dist < 10:
                        ...
                        bonus = 0.5
                    base_damage = max(1, self.player.atk - self.enemy_data.get("def", 0) // 2)
                    damage = int(base_damage * bonus)
                    self._damage_enemy(damage)
                    self._play_sound("enemy_damage")
                    if not self.battle_defeated:
    ...
    def _defeat_enemy(self):
        ...
        exp_gain = self.enemy_data.get("exp", 10)
        gold_gain = self.enemy_data.get("gold", 5)
        ...
        if self.player.lv > 1:
            lv_msg = f"* You're now LV {self.player.lv}."
            self.dialogue_queue = [f"* {self.enemy_data['name']} was defeated.", kill_msg, lv_msg]
        else:
            self.dialogue_queue = [f"* {self.enemy_data['name']} was defeated.", kill_msg]
        self._start_dialogue()
    ...
    def _do_act(self, act_name):
        ...
        elif act_name in ("Compliment", "Console", "Imitate", "Cheer"):
    ...
    def _spare_enemy(self):
        ...
        self.dialogue_queue = [
            f"* {self.enemy_data['name']} was spared.",
    ...
    def _do_flee(self):
        ...
        if random.randint(1, 100) <= chance:
            ...
            self.battle_message_lines = [msg]
            self.battle_message_index = 0
            # End battle after showing flee message
            self.battle_defeated = True  # HACK: to end battle
        else:
            msg = "* Can't escape!"
            self.battle_message_active = True
            self.battle_message_lines = [msg]
    ...
    def _draw_battle(self):
        ...
        # Soul (heart)
        if self.battle_attack_phase:
            self._draw_heart(int(self.soul_x), int(self.soul_y), RED if random.random() > 0.05 else WHITE, self.soul_size)
    ...
    def _draw_pause(self):
        ...
        if hasattr(self, 'pause_message') and self.pause_msg_timer > 0:
        ...
        for e in pygame.event.get():
            if e.type == pygame.QUIT:
                ...
                if e.key in CONFIRM_KEYS:
                    # Retry - reload game
                    self._load_game()
                    self._enter_room(self.room_name)
                    self._set_state(GameState.OVERWORLD)
    ...
    def run(self):
        while self.running:
            ...
            if self.state == GameState.MENU:
                ...
                self._update_battle()
                self._draw_battle()
            elif self.state == GameState.BATTLE_MENU:
                self._update_battle_menu()
                self._draw_battle()
            elif self.state == GameState.BATTLE_ACT:
                self._update_battle_menu()
                self._draw_battle()
            elif self.state == GameState.BATTLE_ITEM:
                self._update_battle_menu()
                self._draw_battle()
            elif self.state == GameState.BATTLE_MERCY:
                self._update_battle_menu()
                self._draw_battle()
            elif self.state == GameState.BATTLE_FIGHT:
                self._update_fight()
                self._draw_fight()
            elif self.state == GameState.BATTLE_DIALOGUE:
                self._update_dialogue()
                self._draw_dialogue()
            ...
            # Blue hue overlay (subtle)
            hue = BLUE_HUE_SURF.copy()
            hue.set_alpha(8)
            self.screen.blit(hue, (0, 0))
